@if (count($blogs) != 0)
  @foreach ($blogs as $blog)
      <div class="section-header mt-3">
        <div class="mb-3">
          <a href="{{route('blog.show', [$blog->slug])}}" class="decoration-none">
          ...
          @php
              // delete tag image;
              $content = preg_replace('/<img .*. \/>|<p.*?.>|<\/p>/', '', $blog->content);
          @endphp
        <p> 
          &emsp;&emsp;&emsp; {!! Str::limit($content, 725, ' . . .') !!}
        </p>
          <a href="{{route('blog.show', [$blog->slug])}}" class="ml-3"> <span class="text-primary">Read More <i class="fa fa-long-arrow-right"></i> </span></a>
        <hr class="mt-3">
      </div>
    @endforeach
@else
